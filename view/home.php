...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['nombre'])) {
    ...
    if (!empty($nombre)) {
        ...
        try {
            $pdo->prepare("INSERT INTO materias (nombre_materia, id_usuario, color) VALUES (?, ?, ?)")
                ->execute([$nombre, $user_id, $color]);
            ob_end_clean();
                ...
                <div class="course-actions" onclick="event.stopPropagation()">
                    <div class="relative">
                        ...
                        <div class="ctx-menu" id="ctx-<?= $m['id_materia'] ?>">
                            ...
                            <div class="ctx-item danger" onclick="eliminarMateria(<?= $m['id_materia'] ?>, '<?= htmlspecialchars(addslashes($m['nombre_materia'])) ?>')">
...
function eliminarMateria(id, nombre) {
    Swal.fire({
        ...
        html: `<span style="color:var(--text-secondary);font-size:14px;">Se eliminará <strong style="color:#eef0f5">${nombre}</strong> y todas sus actividades.</span>`,
        icon: 'warning',
        ...
        if (r.isConfirmed) {
            document.getElementById('delMateriaId').value = id;
            document.getElementById('frmDelMateria').submit();
        }
    });
}
</script>
